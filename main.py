...
def win_chances(a: Player, b: Player, turns: 'int | None' = None):
    ...
    if a.hit_chance == b.hit_chance == 1 and False:
        ...
        a_wins_by = np.fromiter((win_by(a.expr, b.hp, t) for t in range(min_turns, max_turns)),
                                sympy.Rational, max_turns - min_turns)
        ...
        a_wins_by = np.fromiter((win_by_can_miss(a, b.hp, t) for t in range(min_turns, turns)),
                                sympy.Rational, turns - min_turns)
...
def win_chances_initiative(a: Player, b: Player, turns: int):
    ...
    return a_w + ipa * d, b_w + ipa * d


if __name__ == '__main__':
    p0 = Player.from_coeffs(5, (1, 1), sympy.Rational(1, 1), 14)
    p1 = Player.from_coeffs(6, (1, 1), sympy.Rational(1, 1), 1)
    r = win_chances(p0, p1, 5)
    print(r)
    print(1 - sum(r))  # error
    win_chances_initiative(p0, p1, 1)
